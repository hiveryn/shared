...
- Intent approval contract: `Intent` (a pending, approvable agent tool call) and its optional approval-input schema — `IntentInputField` / `IntentInputType` (`text`, `textarea`, `choice`, `boolean`) / `IntentInputOption`, `IntentInputValues`, `IntentInputIssue` (`Intent.unresolved_inputs`: required inputs whose defaults cannot satisfy the schema, which block automatic approval) and `ApproveIntentRequest` (the desktop's `{"inputs": {...}}` approve body).
